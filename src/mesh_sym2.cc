...
vector<patchPair> icpIter(TriMesh* themesh, TriMesh* themesh2, vector<int> &pair1, vector<int> &pair2, vector<bool> &model1, vector<bool> &model2, xform &xf, xform &final){
	...
	//while(pair1.size()>0){ //Iterate until all point coorespondences are tested	
		...
		while((int)patch1->vertices.size()<50 || (int)patch2->vertices.size()<50){
		if(model1.at(i))
		...
		errorPrev=error;
		while((error<icpThresh) && (sizeCheck<(int)patch1->vertices.size())&& (error>0) &&(diffError<1000)){
			//Update Masks
...
int main(int argc, char *argv[])
{
	...
	*/
	...
	for(int c=0; c<samplePoints.size(); c++){
		...
		for (int j = 0; j < nv; ++j) {
			...
			if (abs(ak1-bk1) < eps && abs(ak2-bk2) < eps) {
				...
				//vec t = mesh->vertices[i] - copy->vertices[j]; //kbw
				vec t = copy->vertices[j] - mesh->vertices[i];
	...
	for(int i=0; i<modes.size(); i++){
		...
		clusterPatch = icpIter(mesh, copy, clusterPair1, clusterPair2, clusterModel1, clusterModel2, clusterT, tempTrans);
	...
	for(int i=0; i<clusterPatches.size(); i++){
		...
		for(int j=0; j<clusterPatch.size(); j++){
			...
			printf("Cluster: %d, patch: %d, size: %d, error: %f.\n", i, j, size, clusterPatch.at(j).error);
		}
	}
