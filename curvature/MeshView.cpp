...
BEGIN_MESSAGE_MAP(CMeshView, CView)
	...
	ON_COMMAND(ID_VIEW_KMIN, OnViewKmin)
	ON_UPDATE_COMMAND_UI(ID_VIEW_KMIN, OnUpdateViewKmin)
	ON_COMMAND(ID_VIEW_KMAX, OnViewKmax)
	ON_UPDATE_COMMAND_UI(ID_VIEW_KMAX, OnUpdateViewKmax)
	ON_COMMAND(ID_VIEW_ASYMPTOTIC, OnViewAsymptotic)
	ON_UPDATE_COMMAND_UI(ID_VIEW_ASYMPTOTIC, OnUpdateViewAsymptotic)
END_MESSAGE_MAP()
...
CMeshView::CMeshView()
{
	...
	m_view_kmin = true;
	m_view_kmax = true;
	m_view_asymptotic_directions = false;
}
...
void CMeshView::OnPaint()
{
	...
	{
		::glColor3ub(0,0,255);
		pDoc->m_pMesh->gl_draw_directions(true,false,false,m_view_asymptotic_directions);
	}
	if(m_view_kmax)
	{
		::glColor3ub(255,0,0);
		pDoc->m_pMesh->gl_draw_directions(false,true,false,m_view_asymptotic_directions);
	}

	// asymptotic directions
	if(m_view_asymptotic_directions)
	{
		::glColor3ub(0,255,0);
		pDoc->m_pMesh->gl_draw_directions(false,false,true,false);
	}
...
// view kmin (toggle)
void CMeshView::OnViewKmin()
{
	m_view_kmin = !m_view_kmin;
	InvalidateRect(NULL,FALSE);
}
void CMeshView::OnUpdateViewKmin(CCmdUI *pCmdUI)
{
	pCmdUI->SetCheck(m_view_kmin);
}

// view kmax
void CMeshView::OnViewKmax()
{
	m_view_kmax = !m_view_kmax;
	InvalidateRect(NULL,FALSE);
}
void CMeshView::OnUpdateViewKmax(CCmdUI *pCmdUI)
{
	pCmdUI->SetCheck(m_view_kmax);
}

// view asymptotic directions
void CMeshView::OnViewAsymptotic()
{
	m_view_asymptotic_directions = !m_view_asymptotic_directions;
	InvalidateRect(NULL,FALSE);
}
void CMeshView::OnUpdateViewAsymptotic(CCmdUI *pCmdUI)
{
	pCmdUI->SetCheck(m_view_asymptotic_directions);
}
